...
import { triggerWorkflowRun, approveStep, subscribeToStepRuns } from '@/lib/workflowEngine';
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 3. MUTATION: insert_workflow_steps (Layer 2 Step Gating Test)
    ...
    // 5. QUERY: step_runs (Layer 1 Scoped)
    if (normalizedQuery.includes('step_runs') || normalizedQuery.includes('GetStepRuns')) {
      ...
      // Filter step runs strictly to user's organization
    ...
    // Fallback response for generic query
    return NextResponse.json({
      data: {
